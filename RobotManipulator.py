...
class RobotManipulator:
    ...
    def move(self, x: float, y: float, z: float, a: float, b: float, c: float) -> None:
        """
        Перемещение робота в заданную позицию.

        Args:
            x, y, z: Координаты позиции в мм
            a, b, c: Углы ориентации в градусах
        """
        self._wait_for_idle()
        ...
        ret = self.arm.motion.move_to_pose(motion_pose, const.MOVE_JOINT)
        assert ret == StatusCodeEnum.OK, f"Ошибка перемещения робота: {ret}"
    ...
    def get_position(self) -> dict:
        ...
        Returns:
            Словарь с координатами x, y, z и углами a, b, c
        """
        self._wait_for_idle()
        motion_pose, ret = self.arm.motion.get_current_pose(const.CART, 0, 0)
        assert ret == StatusCodeEnum.OK, f"Ошибка получения позиции робота: {ret}"
        self._wait_for_idle()
        return {
            'x': motion_pose.cartData.position.x,
            'y': motion_pose.cartData.position.y,
            'z': motion_pose.cartData.position.z,
            'a': motion_pose.cartData.position.a,
            'b': motion_pose.cartData.position.b,
            'c': motion_pose.cartData.position.c
        }
    def activate_user_frame(self, frame_id: int):
        """Активирует заданный User Frame для всех последующих движений."""
        print(f"[{self.name}] Активация User Frame ID={frame_id}")
        ret = self.arm.motion.set_param(param_name=ParamType.UF, param_value=frame_id)
        assert ret == StatusCodeEnum.OK, f"Ошибка активации User Frame: {ret}"